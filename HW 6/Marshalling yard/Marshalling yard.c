...
Node* infixToPostfix(char infixExpression[]) {
    ...
    for (int i = 0; i < length; i++) {
        if (infixExpression[i] != ' ' && infixExpression[i] != '\0') {
            ...
            case ')': {
                int errorCode = 0;
                while (top(auxillaryStack) != '(') {
                    char character = top(auxillaryStack);
            ...
            case '*':
            case '/':
                if (top(auxillaryStack) == '+' || top(auxillaryStack) == '-' || top(auxillaryStack) == '(' ||
                    top(auxillaryStack) == 'n') {
                    push(&auxillaryStack, infixExpression[i]);
                }
                else {
                    int errorCode = 0;
            ...
            case '+':
            case '-':
                if ((top(auxillaryStack) != '*' && top(auxillaryStack) != '/') || top(auxillaryStack) == 'n') {
                    push(&auxillaryStack, infixExpression[i]);
                }
                else {
                    int errorCode = 0;
